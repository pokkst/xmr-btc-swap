...
{
    util::print_swap_log_ln(&env, format!("Requesting quote"));
    ...
    util::print_swap_log_ln(&env, format!("Received quote: {}, {}, {}", bid_quote.price, bid_quote.min_quantity, bid_quote.max_quantity));

    let mut max_giveable = max_giveable_fn().await?;
    util::print_swap_log_ln(&env, format!("Amount able to be locked with seller: {}", max_giveable));
    ...
    if max_giveable == bitcoin::Amount::ZERO || max_giveable < bid_quote.min_quantity {
        ...
        loop {
            ...
            util::print_swap_log_ln(&env, format!("New amount able to be locked with seller: {}", max_giveable));
...
pub fn on_asb_btc_balance_data(env: &JNIEnv, data: AsbBtcBalanceData) {
    let listener = get_asb_listener(&env);
    let asb_balance_data_json = serde_json::to_string(&data).unwrap();
    let balance_json_bytes = JObject::from(env.byte_array_from_slice(asb_balance_data_json.as_bytes()).expect("Failed to get swap_id bytes"));
    if let JValue::Object(listener) = listener {
